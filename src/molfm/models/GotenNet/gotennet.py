...
class GotenNetWrapper(GotenNet):
    ...
    def forward(self,batched_data):
        ...
        f_node_fea,f_node_vec = self.valina_forward(
            Data(**{"z":f_atomic_numbers,"pos":f_pos,"batch":f_batch})
            )

        node_features = torch.zeros((B, N, self.hidden_dim), device=device)
        node_vec_features = torch.zeros((B, N, 3, self.hidden_dim), device=device)
    ...
    def valina_forward(self, inputs: Mapping[str, Tensor]) -> Tuple[Tensor, Tensor]:
        """
        Compute atomic representations/embeddings.
