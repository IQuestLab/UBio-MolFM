...
def construct_radius_neighbor(node_pos,node_mask,
    ...
    if error_check:
        ...
        if torch.max(a1)!=0:
            print("error: Please verify your code , neighbor selection error happened")
        if torch.sum(node_pos[~node_mask]==0):
            print("Warninngs, node padding part is zero, will lead to some neighbor dist sort error")
        if torch.sum(expand_node_pos[~expand_node_mask]==0):
            print("Warninngs, expand_node_pos padding part is zero, will lead to some neighbor dist sort error")
