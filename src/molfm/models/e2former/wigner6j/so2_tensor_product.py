...
class E2TensorProductSO2_FirstOrder(torch.nn.Module):
    ...
    def forward(
        ...
        f_N1, topK = alpha_ij_divr.shape[:2]
        ...
        f_sparse_idx_expnode = batched_data["f_sparse_idx_expnode"].contiguous()
        if triton_kernel is not None:
            merged_ab = triton_kernel(value = merged_ab,idx = f_sparse_idx_expnode,alpha = alpha_ij_divr)
...
def test_so2_o1():
    ...
    print(torch.max(torch.abs(h_e2-hij_gt))) #,4:,:10] /hij_gt[0,1,4:,:10])
...
def test_so2_o2():
    ...
    print("max", torch.max(torch.abs(r/h_ab[:,:,:])))
    print("min", torch.min(torch.abs(r/h_ab[:,:,:])))
    print("mean",torch.mean(torch.abs(r/h_ab[:,:,:])))
