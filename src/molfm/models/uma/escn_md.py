...
class eSCNMDBackbone(nn.Module, MOLEInterface):
    ...
    def forward(self,batched_data):
        ...
        atom_masks = batched_data["atom_masks"]
        ...
        f_atomic_numbers = atomic_numbers[atom_masks]     
        f_pos = pos[atom_masks]         
        from torch_cluster import radius_graph
        from torch_geometric.data import Data

        
        f_batch = torch.arange(B).unsqueeze(1).expand(B, N).to(device)[atom_masks]  
        edge_index = radius_graph(f_pos, r=self.cutoff, batch=f_batch, loop=False,
                                        max_num_neighbors=self.max_neighbors)
        edge_vec = f_pos[edge_index[0]] - f_pos[edge_index[1]]

        f_node_irreps = self.vanilla_forward(
                    Data(**{"atomic_numbers":f_atomic_numbers,
                    "pos":f_pos,
                    "batch":f_batch,
                    "charge":batched_data["charge"].view(-1),
                    "spin":batched_data["multiplicity"].view(-1),

                    "edge_distance":torch.norm(edge_vec,dim = -1),
                    "edge_distance_vec":edge_vec/torch.norm(edge_vec,dim = -1,keepdim=True),
                    "edge_index":edge_index,

                    "atomic_numbers_full":f_atomic_numbers,
                    "edge_distance_full":torch.norm(edge_vec,dim = -1),
                    "edge_distance_vec_full":edge_vec/torch.norm(edge_vec,dim = -1,keepdim=True),
        ...
        f_node_fea = f_node_irreps[:,0]
        f_node_vec = f_node_irreps[:,1:4]
        node_features = torch.zeros((B, N, self.sphere_channels), device=device)
        node_vec_features = torch.zeros((B, N, 3, self.sphere_channels), device=device)
        node_features[atom_masks] = f_node_fea
        node_vec_features[atom_masks] = f_node_vec[:,:3] # f_N1* (lmax**-1)*hidden
        return {
    ...
    def vanilla_forward(self, data_dict: AtomicData,g_graph=True) -> dict[str, torch.Tensor]:
