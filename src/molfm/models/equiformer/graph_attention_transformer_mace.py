...
class GraphAttention(torch.nn.Module):
    ...
    def __init__(
        ...
        # For MACE
        self.reshape = reshape_irreps(self.irreps_node_output)
        self.proj_sc = LinearRS(
            self.irreps_node_input,
            self.irreps_node_output,
        )
        self.prod = EquivariantProductBasisBlock(
            node_feats_irreps=self.irreps_node_output,
            target_irreps=self.irreps_node_output,
            correlation=3,
            num_elements=atom_type_cnt,
            use_sc=True,
        )
    ...
    def forward(
        ...
        **kwargs,
        ...
        one_hot_atomic_numbers = torch.nn.functional.one_hot(
            atomic_numbers.long(), self.atom_type_cnt
        )
        node_output = self.prod(
            node_feats=self.reshape(node_output),  # [B, L, D] => [B*L, channels, -1]
            sc=self.proj_sc(node_input),  # [B*L, D]  MACE V0.2
            node_attrs=one_hot_atomic_numbers.float(),
        )
...
class EquiformerMace(torch.nn.Module):
    def __init__(
        self,
