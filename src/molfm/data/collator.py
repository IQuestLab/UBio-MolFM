...
special_pad_keys = {
    "pos": lambda x, pad_len: F.pad(x, (0, 0, 0, pad_len)),              # (N, 3)
    "atomic_numbers": lambda x, pad_len: F.pad(x, (0, pad_len)),         # (N,)
    "fixed": lambda x, pad_len: F.pad(x, (0, pad_len)),         # (N,)
    "forces": lambda x, pad_len: F.pad(x, (0, 0, 0, pad_len)),           # (N, 3)
}
...
class MultiDatasetFixedBatchDistributedSampler(Sampler):
    def __init__(self,
        ...
        if len(sizes) != len(batch_sizes):
            raise ValueError("sizes != batch_sizes")
